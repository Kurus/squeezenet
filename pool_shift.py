...
def model(input_height, input_width, input_channels, output_classes, pooling_size=(1, 3, 3, 1)):
    ...
    # layer 1 - conv 1
        ...
        # layer 2 - maxpool
        ans = tf.nn.max_pool(A_1, ksize=pooling_size, strides=(1, 2, 2, 1), padding='VALID', name='maxpool_1')

        # layer 3-5 - fire modules
        ans = fire_module(ans, 16, 64, 64, layer_num=2, trainable=False)
        ans = fire_module(ans, 16, 64, 64, layer_num=3, trainable=False)
        ans = fire_module(ans, 32, 128, 128, layer_num=4, trainable=False)

        ans = tf.nn.max_pool(ans, ksize=pooling_size, strides=(1, 2, 2, 1), padding='VALID', name='maxpool_4')

        ans = fire_module(ans, 32, 128, 128, layer_num=5, trainable=False)

        # layer 7-10 - fire modules
        ans = fire_module(ans, 48, 192, 192, layer_num=6)
        ans = fire_module(ans, 48, 192, 192, layer_num=7)
        ans = fire_module(ans, 64, 256, 256, layer_num=8)
        ans = tf.nn.max_pool(ans, ksize=pooling_size, strides=(1, 2, 2, 1), padding='VALID', name='maxpool_8')
        ans = fire_module(ans, 64, 256, 256, layer_num=9)


        dropout_9 = tf.cond(in_training,
                            lambda: tf.nn.dropout(ans, keep_prob=0.5),
                            lambda: ans)
